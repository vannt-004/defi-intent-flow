...
class PortfolioAnalyticsService:
    ...
    def __init__(self):
        ...
        self.onchain_transaction_repository = OnchainTransactionRepository(self.db)
    ...
    def get_transactions(self, wallet: str, limit: int = 100) -> list[dict]:
        ...
        onchain_txs = self.onchain_transaction_repository.get_wallet_transactions_chronological(wallet)
        basis_by_symbol = {}
        amount_by_symbol = {}
        enriched = []

        for row in self._merge_events(cashflows, onchain_txs):
            ...
            estimated_price = self.pricing.get_historical_price(symbol, int(row.get("timestamp") or 0))
    ...
    def get_positions_pnl(self, wallet: str) -> list[dict]:
        ...
        for position in positions:
            ...
            position_type = "borrow" if side == "BORROWER" else "lend"
            pnl_type = "interest_cost" if side == "BORROWER" else "interest_earned"
    ...
    def _build_net_worth(self, latest: dict) -> dict:
        return {
            ...
            "lendingUsd": round(float(latest.get("total_supply_usd") or 0), 4),
            "interestEarnedUsd": round(float(latest.get("supply_interest_usd") or 0), 4),
            "debtUsd": round(float(latest.get("total_borrow_usd") or 0), 4),
            "debtInterestUsd": round(float(latest.get("borrow_interest_usd") or 0), 4),
    ...
    def _format_transaction(
        ...
        return {
            ...
            "priceSource": "historical_snapshot" if estimated_price else "recorded_amount",
    ...
    def _estimated_value(self, row: dict) -> float:
        price = self.pricing.get_historical_price(row.get("symbol"), int(row.get("timestamp") or 0))
        amount = float(row.get("amount") or 0)
        if price > 0:
            return amount * price
    ...
    def _token_basis_events(self, wallet: str) -> list[dict]:
        cashflows = self.cashflow_repository.get_wallet_cashflows_chronological(wallet)
        onchain_txs = self.onchain_transaction_repository.get_wallet_transactions_chronological(wallet)
        return self._merge_events(cashflows, onchain_txs)

    def _merge_events(self, cashflows: list[dict], onchain_txs: list[dict]) -> list[dict]:
        events = []

        for row in cashflows:
            item = dict(row)
            item["event_source"] = "protocol"
            item["transaction_category"] = "protocol"
            item["action_label"] = self._action_label(item.get("action"))
            events.append(item)

        for row in onchain_txs:
            item = dict(row)
            item["tx_id"] = row.get("tx_hash")
            item["amount_usd"] = 0.0
            item["market_id"] = row.get("to") or row.get("from")
            item["event_source"] = "onchain"
            events.append(item)
